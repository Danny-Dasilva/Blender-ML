...
def batch_render(file_prefix="render"):
    ...
    scene_setup_steps = 1000
    value = True
    loop_count = 0
    labels = []
    while loop_count != scene_setup_steps:
        outer_port = bpy.data.objects["outer_port"]
        inner_port = bpy.data.objects["inner_port"]
        bottom_port = bpy.data.objects["bottom_port"]
        scene, camera = randomize_camera(4, -5, 1.2)

         
        distance, z = center_obj(camera, outer_port.matrix_world.to_translation())

        offset(scene, camera, 60)
        value, percent = get_raycast_percentage(scene, camera, outer_port, 40)
        ...
        if value == False:
            ...
            bottom, percent = get_raycast_percentage(scene, camera, bottom_port, 40)
            print(percent, "percent in bottom_port")
            inner, percent = get_raycast_percentage(scene, camera, inner_port, 55)
            print(percent, "percent in inner_port")
            if inner and bottom:
                objects = [outer_port, inner_port, bottom_port]
            elif inner:
                objects = [outer_port, inner_port]
            elif bottom:
                objects = [outer_port, bottom_port]
            else:
                objects = [outer_port]
                
            
            scene_labels = get_cordinates(scene, camera, objects, filename)
